...
###Stuck here. I want Substance.x to be columns and sample to be rows and vales to be adj_abs
#These are all columns now. I have tried transposing, melting ,gathering, and spreadin ...

#Transform so that substances are columns and sample is row?
#First select just the columns needed... 

eco_joined_adj_sub <- eco_joined_adj %>%
  select(Substance.x, sample,  adj_abs ) %>%
  mutate(adj_abs = ifelse(adj_abs < 0, 0, adj_abs)) #%>%
  #column_to_rownames(var="Substance.x")
  #mutate(name=paste(sample, Substance.x, sep = "_")) 

df <- as.data.frame(unique(eco_joined_adj_sub[,"adj_abs"]))

eco_joined_adj_sub <- eco_joined_adj_sub %>% 
  spread(key=Substance.x, value=adj_abs)


shannon <- diversity(eco_joined_adj_sub, "shannon")
###Once get shannon add to meta 
