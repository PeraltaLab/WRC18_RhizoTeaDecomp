...
eco_subs <- ggplot(eco_joined_adj, aes(x=source, y=adj_abs, color = treatment))+
  geom_boxplot() + facet_wrap(~ Substance.x, ncol =6)
...
meta <- cbind(meta, u_sample)
#eco_meta <- eco_meta[,-c(31:32)]

#b_meta <- meta[,-c(1)]
#don't need all of meta just treatment and source... 
eco_meta <- eco_joined_adj_sub %>% left_join(b_meta, by="b_sample")  

i <- with(eco_meta, interaction(treatment, source))

ggplot(eco_meta, aes(x=source, y=PA, fill=group)) +
...
nitro_subs <- ggplot(nitro_joined_adj, aes(x=source, y=adj_abs, color = treatment))+
  geom_boxplot() + facet_wrap(~ Substance.x, ncol =6)
...
meta <- cbind(meta, u_sample)
#nitro_meta <- nitro_meta[,-c(31:32)]

#b_meta <- meta[,-c(1)]
#don't need all of meta just treatment and source... 
nitro_meta <- nitro_joined_adj_sub %>% left_join(b_meta, by="b_sample")  

i <- with(nitro_meta, interaction(treatment, source))

ggplot(nitro_meta, aes(x=group, y=PA, fill=source)) +
  geom_bar(stat="identity") +
  #facet_grid(~ source) +
  coord_flip()      
...
po4_rep1 <- po4_T %>%
 filter(Substance != "negative control") %>%
 filter(Well %in% c(1:12), Row %>% c("A","B","C","D","E"))

#sulfur F1-H12
po4_rep2 <- po4_T %>%
 filter(Substance != "water") %>%
 filter(Well %in% c(1:12),Row %>% c("F","G","H"))
...
po4_rep2_water <- po4_T %>%
  filter(Substance == "water",Well==c(5,6,7,8)) %>%
  group_by(sample, plate_num, Substance) 

po4_rep3_water <- po4_T %>%
  filter(Substance == "water",Well==c(9,10,11,12)) %>%
  group_by(sample, plate_num, Substance) 

#Timepoint T04, All substances except water in rep1 (ie wells 1-4 and rows a-h) joined with water value
po4_rep1_join <- po4_rep1 %>% left_join(po4_rep1_water, by=c("sample"))
po4_rep2_join <- po4_rep2 %>% left_join(po4_rep2_water, by=c("sample"))
po4_rep3_join <- po4_rep3 %>% left_join(po4_rep3_water, by=c("sample"))

#Coalesce and then add adjusted abs column  - adj. abs  
po4_joined <- bind_rows(po4_rep1_join, po4_rep2_join, po4_rep3_join)
...
po4_subs <- ggplot(po4_joined_adj, aes(x=source, y=adj_abs, color = treatment))+
  geom_boxplot() + facet_wrap(~ Substance.x, ncol =6)
...
ggsave("../figures/WRC18_Rhizo/po4_shan.jpg", plot=po4_shan, device=NULL, path=NULL, scale=1, width=6.3, height=4.4, dpi=900, limitsize=TRUE)
```

```{r biolog po4 plate substance count}
#For each treatment*source get count of group (group is the category of substrate)
#Presence absence; set a threshold positive value
#Left join substrate group to biolog results to add group to each substrate?
biolog_sub_groups <- read.csv("../data/WRC18_Rhizo/biolog_substrate_groups.csv")
biolog_sub_groups <- biolog_sub_groups %>% 
    filter(plate_type=="po4") %>%
    slice(1:32)
...
meta <- cbind(meta, u_sample)
#po4_meta <- po4_meta[,-c(31:32)]

#b_meta <- meta[,-c(1)]
#don't need all of meta just treatment and source... 
po4_meta <- po4_joined_adj_sub %>% left_join(b_meta, by="b_sample")  

i <- with(po4_meta, interaction(treatment, source))

ggplot(po4_meta, aes(x=source, y=PA, fill=group)) +
  geom_bar(stat="identity") +
  #facet_grid(~ source) +
  coord_flip()      
