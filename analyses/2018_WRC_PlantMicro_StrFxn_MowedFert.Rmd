...
meta$plant.rich <- diversity.plant.even$rich.even
...
#Where do otu data begin, remove for PERMANOVA
...
colnames(meta) #total non-OTU columns is 41
```
...
plot1a + theme(panel.grid.major = element_blank(), 
  ...
  theme(axis.title = element_text(size=14), 
        ...
        panel.border = element_rect(colour = "black", size = 1.25)) + 
  theme(axis.ticks.length = unit(0.3, "cm")) + 
        xlab("PCoA 1 (38.4%)") + ylab("PCoA 2 (23.2)") +
        labs(colour = "Treatment") +
    guides(colour = guide_legend(override.aes = list(pch=16, size = 4)),
...
colnames(metaotu)  #total non-OTU columns is 36

dist.plant <- vegdist(PCC.2018.even[,-c(1:3)], method="bray")
dist.bac <- vegdist(metaotu[,-c(1:36)], method="bray")

#MANTEL - all even blocks
mantel.rtest(dist.plant, dist.bac, nrepet = 999) #all treatments and ditch included - Mantel r = 0.645, p = 0.004
...
dist.bac.M <- vegdist(bac.2018.M[,-c(1:36)], method="bray")
dist.bac.MF <- vegdist(bac.2018.MF[,-c(1:36)], method="bray")

# MANTEL TESTS - even blocks only 
mantel.rtest(dist.plant, dist.bac, nrepet = 999) # mowed only: Mantel r = 0.645, p = 0.003
mantel.rtest(dist.plant.M, dist.bac.M, nrepet = 999) # mowed only: Mantel r = 0.553, p = 0.163
mantel.rtest(dist.plant.MF, dist.bac.MF, nrepet = 999) # mowed/fert: Mantel r = 0.408, p = 0.245
...
cor.test(meta$plant.shannon, meta$bac.shannon) # ALL: Pearsonn rho = -0.926, p = 0.0009
cor.test(meta.M$plant.shannon, meta.M$bac.shannon) # ALL: Pearsonn rho = -0.452, p = 0.548
cor.test(meta.MF$plant.shannon, meta.MF$bac.shannon) # ALL: Pearsonn rho = -0.932, p = 0.068
...
# MANTEL TESTS
mantel.rtest(dist.plant, dist.fungal, nrepet = 999) #ALL - Mantel r = 0.663, p = 0.002
mantel.rtest(dist.plant.M, dist.fungal.M, nrepet = 999) #mowed only: Mantel r = 0.401, p = 0.283
mantel.rtest(dist.plant.MF, dist.fungal.MF, nrepet = 999) #mowed/fert: Mantel r = 0.563, p = 0.074 

# correlation shannon diversity
cor.test(meta$fun.shannon, meta$plant.shannon) # ALL: Pearson rho = 0.183, p = 0.664
cor.test(meta.M$fun.shannon, meta.M$plant.shannon) # mowed only: Pearson rho = -0.070, p = 0.930
...
## fungal/plant/bacterial and carbon metabolic profile
```{r}
...
cor.test(meta$bac.shannon, meta$eco_shannon) # ALL: Pearson rho = -0.030, p = 0.943
...
carbon.shannon<-p1 + theme_bw() + 
    ...
    theme(strip.text.x = element_text(size=14, face="bold"), strip.text.y =   
          ...
          fill="white", size=1)) +
    scale_x_discrete(breaks=c("M", "MF"), labels=c("unfertilized", "fertilized")) +
    ggtitle("Carbon Metabolic Diversity (2018 - wet)")
carbon.shannon
