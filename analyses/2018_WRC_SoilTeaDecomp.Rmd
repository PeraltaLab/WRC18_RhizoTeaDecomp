...
p <- ggplot(TBI, aes(x=S, y=k, color=trt, shape=ditch))+ scale_color_manual(name="Treatment", values=c("gray70","darkgreen"), labels = c("mowed", "mowed/fertilized")) + geom_point(size=5) + scale_shape_manual(name="Ditch", values = c(16,21), labels = c("wet_0ditch","dry_1ditch"))
p1=p+geom_point()
...
TBI
ggsave("../figures/TBI.jpg", plot=last_plot(), device=NULL, path=NULL, scale=1, width=7, height=5, dpi=300, limitsize=TRUE)
```
...
p <- ggplot(tea, aes(x=source, y=Percent_Loss, color=as.factor(treatment)))+ scale_color_manual(name="Treatment", values=c("gray70", "darkgreen"), labels = c("mowed", "mowed/fertilized")) + stat_summary(fun.data=mean_cl_boot,size=0.75) 
...
ggsave("../figures/PercentLoss_pointCI.png", plot=last_plot(), device=NULL, path=NULL, scale=1, width=7, height=5, dpi=300, limitsize=TRUE)
...
p <- ggplot(tea, aes(x=source, y=Percent_Loss, color=as.factor(treatment))) + geom_boxplot() +
      ...
      scale_color_manual(name="Treatment", values=c("gray70", "darkgreen"), labels = c("mowed", 
      "mowed/fertilized")) 
p1=p+geom_smooth(method="lm")+facet_wrap(~ditch)+facet_grid(. ~ ditch,labeller = labeller(ditch=ditch.labs))
p1 + theme_bw() + 
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line 
...
ggsave("../figures/PercentLoss_boxplots.jpg", plot=last_plot(), device=NULL, path=NULL, scale=1, width=7, height=5, dpi=300, limitsize=TRUE)
```
...
write.csv(t(by_treatment), file = "../figures/soil_properties_trt.csv",row.names=TRUE)
...
by_treatment_ditch <- meta.env %>%
  ...
  summarise_at(c("temp_degC", "moisture", "pH", "c_percent", "n_percent", "C_N", 
                 "n_nh4_ug_gsoil", "n_no3_ug_gsoil", "p_po4_hcl_ug_gsoil", 
                 "p_po4_w_ug_gsoil"), funs(mean,sd), na.rm = TRUE)
write.csv(t(by_treatment_ditch), file = "../figures/soil_properties_trt_ditch.csv",row.names=TRUE)
